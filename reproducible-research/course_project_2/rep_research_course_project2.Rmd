...
stormData <-    select(stormData, EVTYPE, FATALITIES, INJURIES, PROPDMG, CROPDMG, PROPDMG_CROPDMG_MULTIPLIER) %>% 
                ...
                mutate(TOTAL_DOLLAR_VALUE_FOR_PROP_AND_CROP_DAMAGE = PROPDMG_DOLLAR_VALUE + CROPDMG_DOLLAR_VALUE) %>%
                select(EVTYPE, FATALITIES_AND_INJURIES_COMBINED, TOTAL_DOLLAR_VALUE_FOR_PROP_AND_CROP_DAMAGE)
                

                    ##    %>% 
                    ##    group_by(EVTYPE) %>% 
                    ##    summarize(countFatalities = sum(FATALITIES), countInjuries = sum(INJURIES), countFatalitiesAndInjutiesCombined = sum(fatalitiesAndInjuriesCombined)) %>% 
                    ##    arrange(desc(countFatalities))
```
